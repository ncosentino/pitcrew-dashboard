...
function Test-PublicTcpConnection {
    ...
    try {
        $task = $client.ConnectAsync('example.com', 443)
...
function Write-NetworkProbeScript {
...
try {
    $task = $client.ConnectAsync('example.com', 443)
    $connected = $task.Wait([TimeSpan]::FromSeconds(8)) -and $client.Connected
    [IO.File]::WriteAllText($ResultPath, $(if ($connected) { 'connected' } else { 'denied' }))
} catch {
    [IO.File]::WriteAllText($ResultPath, 'denied')
} finally {
    $client.Dispose()
}
'@,
...
function Invoke-InstalledBrokerNetworkDenialProbe {
    ...
    try {
      if ($IsWindows) {
        ...
        try {
            $probeCommand =
                "`"$pwshPath`" -NoLogo -NoProfile -NonInteractive -File `"$probePath`" -ResultPath `"$resultPath`""
...
function Remove-HostTestResidue {
    ...
    if ($IsWindows) {
        ...
        Remove-NetFirewallRule `
            -Name 'PitCrewSupportBroker-Outbound-Block' `
            -ErrorAction SilentlyContinue
    } else {
...
try {
    ...
    Invoke-WebRequest `
        -Uri (
            ...
            '4d30a031/plugins/pitcrew-operations/skills/' +
            'pitcrew-remote-diagnostics/scripts/' +
            'Collect-PitCrewDiagnostics.ps1'
        ) `
    ...
    Add-Check (
        $aclDriftDetected
    ) 'Atomic projection replacement ACL drift was not reported.'
