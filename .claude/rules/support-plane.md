...
- The broker has no outbound network. Windows owns an enabled broker-service firewall block covering the collector. Linux requires `PrivateNetwork=true`, `AF_UNIX` only, and no capabilities. Verify effective systemd state and reject drop-ins.
