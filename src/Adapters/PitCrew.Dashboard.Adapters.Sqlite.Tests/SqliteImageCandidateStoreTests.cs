...
public sealed class SqliteImageCandidateStoreTests
{
      ...
      CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      await Assert.That(migrationVersion).IsEqualTo(33);
  ...
  [Test]
  public async Task Migrations_25_Through_33_Upgrade_Exact_Migration_24_And_Preserve_Checksums(
      CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      await Assert.That(priorChecksums[23])
          .IsEqualTo(OriginMainMigration23Checksum);
      await Assert.That(afterChecksums.Keys.Max()).IsEqualTo(33);
