...
internal sealed class LinuxInstalledCanaryNode : IInstalledCanaryNode
{
  ...
  private readonly LinuxCanaryAgentStatusObserver _statusObserver;
      ...
      string runId)
  {
    ...
    _statusObserver = new LinuxCanaryAgentStatusObserver(
        AgentStateRoot,
        _commands,
  ...
  public Task WaitForAcceptedPollAsync(
      CancellationToken cancellationToken) =>
      _statusObserver.WaitForAcceptedPollAsync(
          cancellationToken);
  ...
  public Task<string?> ReadRequestDispositionAsync(
      CancellationToken cancellationToken) =>
      _statusObserver.ReadRequestDispositionAsync(
          cancellationToken);

  public Task<string?> ObserveRequestFailureAsync(
      CancellationToken cancellationToken) =>
      _statusObserver.ObserveRequestFailureAsync(
          cancellationToken);

  public Task PrepareRequestObservationAsync(
      CancellationToken cancellationToken) =>
      _statusObserver.PrepareRequestObservationAsync(
          cancellationToken);
  ...
  private async Task<bool> HasExistingInstallationAsync(
      ...
      File.Exists(LinuxCanaryConnectorFixture.Root) ||
